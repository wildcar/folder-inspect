...
type ConsoleOptions struct {
	PerCategory int    // findings listed per category (0 = 10)
...
func (r *Report) PrintConsole(w io.Writer, opt ConsoleOptions) {
	...
	if len(r.Summary) == 0 {
		...
		for _, s := range r.Summary {
			fmt.Fprintf(tw, "  %s\t%d\t%s\t\n", i18n.T("cat."+string(s.Category)), s.Count, HumanSize(s.Size))
		}
		tw.Flush()
		fmt.Fprintln(w)
...
func (r *Report) printCategory(w io.Writer, c detect.Category, limit int) {
	...
	fmt.Fprintf(w, "%s (%d)\n", i18n.T("cat."+string(c)), len(items))
	for i, f := range items {
		if i >= limit {
			fmt.Fprintln(w, "  "+i18n.Tf("scan.more", len(items)-i))
			break
		}
		fmt.Fprintf(w, "  %10s  %s%s\n", HumanSize(f.Size), displayPath(f.Root, f.Rel, f.Path), tag(f))
	}
	fmt.Fprintln(w)
}

// tag renders the bracketed qualifier after a finding: rule, threshold,
// translated detail — or nothing when there is nothing to say.
func tag(f detect.Finding) string {
	var parts []string
	if f.Rule != "" {
...
func printTop(w io.Writer, title string, items []Item, limit int) {
	...
	for i, it := range items {
		...
		fmt.Fprintf(w, "  %10s  %s\n", HumanSize(it.Size), displayPath(it.Root, it.Rel, it.Path))
	}
	fmt.Fprintln(w)
}

// displayPath shows the path relative to its root when there is one root
// worth of context; the root itself is printed in the header.
func displayPath(root, rel, path string) string {
	if rel == "" {
		return path
	}
	return rel
